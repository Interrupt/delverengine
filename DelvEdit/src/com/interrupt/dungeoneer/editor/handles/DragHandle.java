...
public class DragHandle extends Handle {
    public final Vector3 position = new Vector3();

    private final Color color = Color.WHITE.cpy();
    private final Color highlightColor = Color.WHITE.cpy();

    public DragHandle(Vector3 position, Vector3 eulerAngles) {
        super();
        position.set(position);
    }

    public DragHandle(Vector3 position, Quaternion rotation) {
        super();
        this.position.set(position);
    }

    public DragHandle(float x, float y, float z) {
        super();
        position.set(x, y, z);
    }
