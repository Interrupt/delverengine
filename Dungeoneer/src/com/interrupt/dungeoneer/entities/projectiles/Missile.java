...
public class Missile extends Item implements Directional {
	...
	public boolean lineCollidesWithLevel(Vector3 pos, Vector3 moveVector, Level level) {
	...
	public boolean canHit(Entity checking, Entity e) {
		...
		else if(checking instanceof Model && checking.isStatic) return false; // don't let missiles collide with static meshes
		else if(checking != null && checking.ignorePlayerCollision && Game.instance.player != null && e == Game.instance.player) return false;
		else if(checking != null && e.ignorePlayerCollision && Game.instance.player != null && checking == Game.instance.player) return false;
		else if(checking != null && e.isDynamic && checking.collidesWith == CollidesWith.staticOnly) return false;
		else if(checking != null && e.collidesWith == CollidesWith.staticOnly && checking.isDynamic) return false;
		else if(checking != null && e.collidesWith == CollidesWith.nonActors && checking instanceof Actor) return false;
		else if(checking != null && e.collidesWith == CollidesWith.actorsOnly && !(checking instanceof Actor)) return false;
		else if(checking != null && checking.collidesWith == CollidesWith.nonActors && e instanceof Actor) return false;
		else if(checking != null && checking.collidesWith == CollidesWith.actorsOnly && !(e instanceof Actor)) return false;
		return true;
	}

	private transient Array<Entity> entityHitList = new Array<Entity>();
	private transient Vector3 entityHitIntersection = new Vector3();
	public Array<Entity> lineCollidesWithEntities(Vector3 pos, Vector3 nextPos, Vector3 moveVector, Level level, Vector3 levelHitLocation) {
		Array<Entity> possibles = level.getEntitiesAlongLine(pos.x, pos.z, nextPos.x, nextPos.z);
		entityHitList.clear();

		for(Entity e : possibles) {
			if(e.isActive && e.isDynamic && canHit(this, e)) {
				BoundingBox b = CachePools.getAABB(e);
				ray.set(pos, tempVec.set(moveVector).nor().scl(-1f));

				if (Intersector.intersectRayBounds(ray, b, entityHitIntersection)) {
					if(entityHitIntersection.equals(Vector3.Zero)) {
						// Uhoh, we're in the entity. Back up a step.
						x -= xa;
						y -= ya;
						z -= za;
						entityHitIntersection.set(nextPos);
					}

					Vector3 hitDistVect = new Vector3(entityHitIntersection);
					float hitDistance = hitDistVect.sub(nextPos).len();

					if(hitDistance <= moveVector.len()) {
						if (levelHitLocation != null) {
							// check if we're closer than the world hit location
							float entityHitDistance = hitDistVect.set(entityHitIntersection).sub(pos).len();
							float worldHitDistance = hitDistVect.set(levelHitLocation).sub(pos).len();
							if (entityHitDistance < worldHitDistance) {
								entityHitList.add(e);
							}
						} else {
							entityHitList.add(e);
	...
	public void tick(Level level, float delta)
	{
		...
		if(!stuck && collidesWith == CollidesWith.nonActors) collidesWith = CollidesWith.staticOnly;

		setDirection();

		if(!stuck) {
			boolean hitLevel = lineCollidesWithLevel(lastPosition, lastPositionToNewPosition, level);
			Array<Entity> colliding = lineCollidesWithEntities(lastPosition, nextPos, lastPositionToNewPosition, level, hitLevel ? levelIntersection : null);
