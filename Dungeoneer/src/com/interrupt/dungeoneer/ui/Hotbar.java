			...
			for(int x = 0; x < columns; x++)
			{
				...
				if(itemButtons.containsKey(i)) {
					...
					if(hb.isDragging)
					{
						if(Game.isMobile || !Game.instance.input.caughtCursor)
						{	
							if((isDragging.containsKey(i) && isDragging.get(i) ) || Math.abs(input.getUiTouchPosition().x - input.getPointerX(uiTouchPointer)) > uiSize / 8 || Math.abs(input.getUiTouchPosition().y - input.getPointerY(uiTouchPointer)) > uiSize / 8) {
					...
					else
					{
						...
						if(wasPressedLast.containsKey(i) && wasPressedLast.get(i)) {
							DragAndDropResult movedItem = Game.DragAndDropInventoryItem( Game.instance.player.inventory.get(i), i, null);
