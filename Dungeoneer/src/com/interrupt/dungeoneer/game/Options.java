...
public class Options {
	...
	public static void loadOptions() {
		try {
			...
			if(Gdx.app != null) type = Gdx.app.getType();

			Options.instance = new Options(type);
			if(Gdx.app != null) Gdx.app.log("Game", "Couldn't load options", e);
		}
	}
	...
	private static void SetGamepadAction(Action action, GamepadBinding binding, GamepadDefinition defaultGamepad) {
		if(binding != null) {
			...
			if(defaultGamepad.gamepadBindings != null) {
				GamepadBinding defaultBinding = defaultGamepad.gamepadBindings.get(action);
	...
	public static void saveOptions() {
		try {
			if(Options.instance != null) {
				...
				String optionsDirString = getOptionsDir();
	...
	{
		if(Gdx.app == null || (Gdx.app.getType() != ApplicationType.Android && Gdx.app.getType() != ApplicationType.iOS)) {

			// TODO: OS SPECIFIC SAVE PATHS GO HERE
			/*if(OSUtils.isWindows()) {
				try {
					return Gdx.files.external("My Documents/My Games/Delver/" + file);
				} catch (Exception ex) {
					Gdx.app.log("DelverLifeCycle", "Couldn't find My Games folder");
				}
			}*/
