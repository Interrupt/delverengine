...
public class CachePools {
    ...
    public static Particle getParticle(Particle particle) {
        Particle copy = particles.obtain();

        copy.isActive = particle.isActive;
        copy.tex = particle.tex;
        copy.artType = particle.artType;
        copy.isSolid = particle.isSolid;
        copy.xa = particle.xa;
        copy.ya = particle.ya;
        copy.za = particle.za;
        copy.isDynamic = particle.isDynamic;
        copy.collision.set(particle.collision);
        copy.floating = particle.floating;
        copy.x = particle.x;
        copy.y = particle.y;
        copy.z = particle.z;
        copy.lifetime = particle.lifetime;
        copy.checkCollision = particle.checkCollision;
        copy.color.set(particle.color);
        copy.scale = particle.scale;
        copy.startScale = particle.startScale;
        copy.endScale = particle.endScale;
        copy.yOffset = particle.yOffset;
        copy.stopAnimation();
        copy.physicsSleeping = particle.physicsSleeping;
        copy.initialized = particle.initialized;
        copy.roll = particle.roll;
        copy.rotateAmount = particle.rotateAmount;
        copy.movementRotateAmount = particle.movementRotateAmount;
        copy.haloMode = particle.haloMode;
        copy.shadowType = particle.shadowType;
        copy.drawDistance = particle.drawDistance;
        copy.turbulenceMoveModifier = particle.turbulenceMoveModifier;
        copy.turbulenceAmount = particle.turbulenceAmount;
        copy.blendMode = particle.blendMode;
        copy.shader = particle.shader;
        copy.startColor = particle.startColor;
        copy.endColor = particle.endColor;

        if (copy.drawable != null) {
            copy.spriteAtlas = particle.spriteAtlas;
            copy.drawable.refresh();
        }

        usedParticles.add(copy);
