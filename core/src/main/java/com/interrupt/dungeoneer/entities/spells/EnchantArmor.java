...
public class EnchantArmor extends Spell {
    ...
    public void doCast(Entity owner, Vector3 direction, Vector3 position) {
        ...
        if(owner instanceof Player) {
            ...
            Array<Armor> found = new Array<Armor>();
            for(String s : p.equippedItems.keySet()) {
                Item i = p.equippedItems.get(s);
                if(i instanceof Armor) found.add((Armor)i);
            }
            found.shuffle();

            if(found.size > 0) {
                Armor picked = found.first();
                if(picked.prefixEnchantment == null) {
