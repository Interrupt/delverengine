...
float calcAttachment() {
  ...
  return 1.0 - mod(v_texCoords.y, mod_height) / mod_height;
}
...
void main() {
  ...
  float offsetTime = u_time - 0.3;
  float smallMovement = sin(offsetTime * 3.0 + a_position.x * 2.0) * 0.5;
  float bigMovement = sin(offsetTime * 1.2 + a_position.x * 2.0) * 0.5 + 0.2;
  float gust = sin(offsetTime * 0.8 + a_position.x * 1.0) * 0.5 + 0.5;
  gust *= gust;

  float swayMod = clamp(0.0, 0.15, calcAttachment());

  newPos.x += (gust * 0.1 + smallMovement * 0.05 + bigMovement * 0.1) * swayMod;
  newPos.z += (gust * 0.05 + smallMovement * 0.04 + bigMovement * 0.1) * swayMod;
