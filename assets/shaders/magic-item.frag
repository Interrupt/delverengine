...
void main() {
    ...
    outline += (outlineMod * (1.0 + sin(v_eyeDistance + u_time * 10.0 + pixelLoc.x * 300.0 + pixelLoc.y * 400.0))) * 0.1 * u_magic_color.a;
    outline *= (1.0 + cos(u_time + pixelLoc.x * 50.0));

    color.r += outline * u_magic_color.r;
    color.g += outline * u_magic_color.g;
